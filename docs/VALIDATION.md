...
- Proyecto Supabase dedicado: 25 tablas con RLS, 17 métodos sembrados, Storage privado, dos Edge Functions y URLs de Auth configuradas. Una solicitud a la API sin sesión devuelve 401. Los asesores muestran 0 errores y 0 advertencias de seguridad y rendimiento.
- Retención: `compa-chat-retention` está activo en `pg_cron` y ejecuta diariamente la purga de mensajes con más de 30 días.
...
| ----------------- | ----------------------------------------------------------------------------------- |
| Supabase real     | Repetir aislamiento extremo a extremo con dos cuentas reales                       |
...
No se declara la beta lista para alumnos mientras estas comprobaciones falten. La web privada publicada ya usa el backend de Supabase; la IA, el worker, los recordatorios push programados y las compilaciones firmadas siguen pendientes.
